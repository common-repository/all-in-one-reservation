	...
	$.fn.dateRangePicker = function(opt) {
		...
		function init_datepicker() {
			...
			function gotoNextMonth(self) {
				...
				if ( ! opt.singleMonth && ! opt.singleDate && ! isMonth2 && compare_month( month, opt.month2 ) >= 0 || isMonthOutOfBounds( month )) {
					return;
				}
			...
			box.find( '.time2 input[type=range]' ).on(
				'change touchmove',
				function(e) {
					var target = e.target,
					hour       = target.name == 'hour' ? $( target ).val().replace( /^(\d{1})$/, '0$1' ) : undefined,
		...
		function updateSelectableRange() {
			...
			if (opt.start && ! opt.end) {
				box.find( '.day.toMonth.valid' ).each(
					function() {
		...
		function bindEvents() {
			...
			box.find( '.day' ).off( "mouseenter" ).on(
				'mouseenter',
				function(evt) {
					dayHovering( $( this ) );
				}
			...
			box.find( '.day' ).off( "mouseleave" ).on(
				'mouseleave',
				function(evt) {
					box.find( '.date-range-length-tip' ).hide();
					if (opt.singleDate) {
			...
			box.find( '.week-number' ).off( "click" ).on(
				'click',
				function(evt) {
		...
		function createDom() {
			...
			var arrowPrev = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M15.293 3.293 6.586 12l8.707 8.707 1.414-1.414L9.414 12l7.293-7.293-1.414-1.414z"/></svg>';
			if (opt.customArrowPrevSymbol) {
				arrowPrev = opt.customArrowPrevSymbol;
			}

			var arrowNext = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24"><path d="M7.293 4.707 14.586 12l-7.293 7.293 1.414 1.414L17.414 12 8.707 3.293 7.293 4.707z"/></svg>';
			...
			for (var week = 0; week < 6; week++) {
				...
				for (var day = 0; day < 7; day++) {
					...
					var todayDivAttr = {
						...
						'class': 'day ' + today.type + ' ' + today.extraClass + ' ' + (today.valid ? 'valid' : 'invalid') + ' ' + (highlightToday ? rt : '')
					};
		...
		function translate(translationKey) {
			...
			if (result == null) {
				result = (translationKey in defaultLanguage) ? defaultLanguage[translationKey] : (translationKeyLowerCase in defaultLanguage) ? defaultLanguage[translationKeyLowerCase] : '';
			}
