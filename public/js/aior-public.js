...
var aior_public = {
	...
	pay_accordian: function() {
		var acc = document.getElementsByClassName( "aior_pay_accordion" );
		var i;
	...
	form_settings: function() {
		obj = '';
		obj = Hooks.apply_filters( 'aior_public_settings', obj );
		if (obj) {
	...
	form_setup: function() {
		$   = jQuery;
		set = this.form_settings();

		// Datepicker range
		$( "#rf_booking_date" ).attr( 'type', 'text' )
		...
		if ($( '#rf_booking_date' ).length > 0) {
			$( '#rf_booking_date' ).after( '<div id="date-range12-container"></div>' );
			$( "#rf_booking_date" ).dateRangePicker(
				{
					dateFormat: set.date_format ? set.date_format : 'mm-dd-yyyy',
					singleMonth: true,
					singleDate: true,
					startDate: new Date(),
					inline: true,
			...
			var time_format      = set.clock_hour ? set.clock_hour : 'hh:mm p';
			var time_interval    = set.time_slot ? set.time_slot : '30';
			var opening_time     = $( '#rf_booking_time_from' );
			var closing_time     = $( '#rf_booking_time_to' );
			var reservation_from = $( '#reservation_from' );
	...
	form_submit: function() {
		...
		$( document ).on(
			...
			'.aior_book_now',
			function() {
				id            = $( this ).attr( 'data-submit-id' );
				form          = '#aior_reservation_form_' + id;
				redirect      = $( this ).attr( 'data-redirect' );
				var validator = $( form ).validate();
				$( form + ' .aior_fomr_filed' ).each(
					function() {
						msg = $( this ).attr( 'data-err-msg' );
						min = $( this ).attr( 'data-err-msg' );
						if (msg) {
							$( this ).rules( "add", { messages: { required: msg } } )
						}
					}
				);
						...
						$.ajax(
							{
								...
								success: function(re) {
									...
									if (re.status == 'success') {
										if (re.ri == 'lb') {
											Swal.fire( { icon: 'success', html: re.message } )
										} else {
											$( '#aior_reservation_form_' + re.data ).html( '<div class="aior_form_success">' + re.message + '</div>' );
										}
									}
								}
							}
...
! function(a) { "function" == typeof define && define.amd ? define( ["jquery"], a ) : "object" == typeof module && module.exports ? module.exports = a( require( "jquery" ) ) : a( jQuery ) }(function(a) {
	a.extend(
		a.fn,
		{
			validate: function(b) {
				...
				return c ? c : (this.attr( "novalidate", "novalidate" ), c = new a.validator( b, this[0] ), a.data( this[0], "validator", c ), c.settings.onsubmit && (this.on( "click.validate", ":submit", function(b) { c.submitButton = b.currentTarget, a( this ).hasClass( "cancel" ) && (c.cancelSubmit = ! 0), void 0 !== a( this ).attr( "formnovalidate" ) && (c.cancelSubmit = ! 0) } ), this.on(
					"submit.validate",
					function(b) {
	...
	a.extend( a.expr.pseudos || a.expr[":"], { blank: function(c) { return ! b( "" + a( c ).val() ) }, filled: function(c) { var d = a( c ).val(); return null !== d && ! ! b( "" + d ) }, unchecked: function(b) { return ! a( b ).prop( "checked" ) } } ), a.validator = function(b, c) { this.settings = a.extend( ! 0, {}, a.validator.defaults, b ), this.currentForm = c, this.init() }, a.validator.format = function(b, c) { return 1 === arguments.length ? function() { var c = a.makeArray( arguments ); return c.unshift( b ), a.validator.format.apply( this, c ) } : void 0 === c ? b : (arguments.length > 2 && c.constructor !== Array && (c = a.makeArray( arguments ).slice( 1 )), c.constructor !== Array && (c = [c]), a.each( c, function(a, c) { b = b.replace( new RegExp( "\\{" + a + "\\}", "g" ), function() { return c } ) } ), b) }, a.extend(
		a.validator,
		{
			...
			messages: { required: "This field is required.", remote: "Please fix this field.", email: "Please enter a valid email address.", url: "Please enter a valid URL.", date: "Please enter a valid date.", dateISO: "Please enter a valid date (ISO).", number: "Please enter a valid number.", digits: "Please enter only digits.", equalTo: "Please enter the same value again.", maxlength: a.validator.format( "Please enter no more than {0} characters." ), minlength: a.validator.format( "Please enter at least {0} characters." ), rangelength: a.validator.format( "Please enter a value between {0} and {1} characters long." ), range: a.validator.format( "Please enter a value between {0} and {1}." ), max: a.validator.format( "Maximum available spaces are {0}." ), min: a.validator.format( "Please enter a value greater than or equal to {0}." ), step: a.validator.format( "Please enter a multiple of {0}." ) },
			autoCreateRanges: ! 1,
			prototype: {
				init: function() {
					function b(b) {
				...
				checkForm: function() { this.prepareForm(); for (var a = 0, b = this.currentElements = this.elements(); b[a]; a++) {
						this.check( b[a] );
				} return this.valid() },
				element: function(b) {
				...
				focusInvalid: function() { if (this.settings.focusInvalid) {
						try {
											a( this.findLastActive() || this.errorList.length && this.errorList[0].element || [] ).filter( ":visible" ).trigger( "focus" ).trigger( "focusin" ) } catch (b) {
											}
				} },
			...
			normalizeRules: function(b, c) {
				...
				), a.each( b, function(a, d) { b[a] = "function" == typeof d && "normalizer" !== a ? d( c ) : d } ), a.each( ["minlength", "maxlength"], function() { b[this] && (b[this] = Number( b[this] )) } ), a.each(
					["rangelength", "range"],
					function() {
						var a;
			...
			methods: {
				...
				remote: function(b, c, d, e) {
					...
					return this.settings.messages[c.name] || (this.settings.messages[c.name] = {}), i.originalMessage = i.originalMessage || this.settings.messages[c.name][e], this.settings.messages[c.name][e] = i.message, d = "string" == typeof d && { url: d } || d, h = a.param( a.extend( { data: b }, d.data ) ), i.old === h ? i.valid : (i.old = h, f = this, this.startRequest( c ), g = {}, g[c.name] = b, a.ajax(
						a.extend(
							! 0,
							{
								mode: "abort",
...
(function($) {
	...
	$( document ).ready(
		function() {
			$( '#reservation_from' ).timepicker();
			$( '#reservation_to' ).timepicker();
		}
	);
	/**
